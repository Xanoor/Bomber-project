...
class Fantome:
    ...
    def createFantom(self, x, y):
        ...
        if len(pos) > 1: # Si la prise n'est pas bloquée:
            ...
            self.x = pos[0]
            self.y = pos[1]
            self.obj = g.afficherImage(pos[0], pos[1], (SIZE.__round__(), SIZE.__round__()), game.IMAGES["F"])
            self.id = self.obj.id
...
class Bombe:
    ...
    def createBomb(self, x, y):
        ...
        Return:
            None
        """
        self.obj = g.afficherImage(x, y, (SIZE.__round__(), SIZE.__round__()), game.IMAGES["B"])
        self.id = self.obj.id
        game.objects[(self.x, self.y, self.id)] = self
    ...
    def getExplosionPattern(self, x, y, dirx=1, diry=0, dist=1, s=0):
        ...
        for i in range(1, dist+1):
            ...
            hasTypeC = any(element.type == "C" for element in getCase(position[0], position[1])) # Vérification si un des types est une colonne
            if hasTypeC: break #Si la position contient une colonne, on s'arrete et n'ajoutons pas la position.
...
class Upgrade:
    ...
    def createUpgrade(self, x, y):
        ...
        Return:
            None
        """
        self.obj = g.afficherImage(x, y, (SIZE.__round__(), SIZE.__round__()), game.IMAGES["U"])
        self.id = self.obj.id
        game.objects[(self.x, self.y, self.id)] = self
...
gameMap, game.timer, game.timerfantome, SIZE = load_map("map0.txt", LARGEUR, HAUTEUR) #récupère la carte, le timer, le timerfantome et la taille des cellules
TIMERFANTOME = game.timerfantome # constante temps d'apparaitions fantomes
game.objects, player, upgrades = initialize_objects(gameMap, g, SIZE, game.IMAGES) # Retourne les objets par défauts et les coordonnées du joueur
